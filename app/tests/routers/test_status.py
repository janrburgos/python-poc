...
def mock_openai_client():
    with patch('app.routers.status.client') as mock_client:
        mock_response = MagicMock()
        mock_response.choices[0].message.tool_calls[0].function.arguments = json.dumps({
            "classified_statuses": [
                {"status_name": "shipment has been cancelled", "status_type": "Exception", "substatus_type": "Cancelled"},
                {"status_name": "package is in transit", "status_type": "Transit", "substatus_type": None}
            ]
        })
